...
export class XtensaToolchain extends BaseToolchain {
  ...
  async setup(): Promise<void> {
    ...
    await this.execCommand('sudo', ['apt-get', 'update', '-q']);
    ...
    if (fs.existsSync(ESP_OPEN_SDK_DIR)) {
      core.info(`Removing existing esp-open-sdk directory (incomplete installation)...`);
      fs.rmSync(ESP_OPEN_SDK_DIR, { recursive: true, force: true });
    }
    ...
    await this.execCommand('make', [], {
      ...
      env: {
        ...process.env,
        PWD: ESP_OPEN_SDK_DIR, // Makefile uses $(PWD) for TOP; must match cwd
