...
// Use direct blob URL to avoid redirect issues with tc.downloadTool
const ARM_TOOLCHAIN_URL = `https://armkeil.blob.core.windows.net/developer/Files/downloads/gnu/${ARM_TOOLCHAIN_VERSION}/binrel/arm-gnu-toolchain-${ARM_TOOLCHAIN_VERSION}-x86_64-arm-none-eabi.tar.xz`;
...
export class ARMToolchain extends BaseToolchain {
  ...
  async setup(): Promise<void> {
    ...
    // Clean up any existing directory (may exist from partial cache restore)
    if (fs.existsSync(this.toolchainPath)) {
      core.info(`Removing existing ARM toolchain directory (incomplete installation)...`);
